...
set.seed(1234)
tru.b.mu = c(7,-1.75) #lower y-value implies lower pain level so treatment effect is significant
...
sims = 1000
p0 = matrix(0,length(designs),sims)
k0 = matrix(0,length(designs),sims)
...
for (j in 1:sims){
  ...
  for (i in 1:3){
    ...
    save(out, file=paste("OP2_",designs[i],"_run",j,".RData",sep=""))
    
    q=post_samples$beta_mu[,2]
    ci_hdi <- ci(q, method = "HDI",ci=0.95) #posterior credible interval
    aa = ci_hdi$CI_low
    bb = ci_hdi$CI_high
    if (bb<0){p0[i,j]=0} else{p0[i,j]=1}
    if (bb<(-0.75)){k0[i,j]=0} else{k0[i,j]=1} #Type II error
  }
}

type2error = rowSums(p0) + rowSums(k0)
power = 1 - type2error/sims
rbind(designs,power) 
